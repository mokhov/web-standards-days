...
main() {
    ...
    local sshFileName=""
    local sshHostName="github.com"
    local sshUserName="travis"
        ...
        case $1 in
            ...
            --H|--hostname)
                 if [ -n "$2" ]; then
                    ...
                    print_error "ERROR: A non-empty \"--H|--hostname <hostname>\" argument needs to be specified"
                    exit 1
                 fi
            ;;
            ...
            --u|--username)
                 if [ -n "$2" ]; then
                    ...
                    print_error "ERROR: A non-empty \"--u|--username <username>\" argument needs to be specified"
                    exit 1
                 fi
            ;;
    ...
    decrypt_private_ssh_key "$key" "$iv" "$pathEncryptedKey" "$sshFileName" \
        &> >(print_error_stream) \
        1> /dev/null
    print $? "Decrypt the file containing the private key" \
        || exit 1

    add_ssh_configs "$sshFileName" "$sshHostName" "$sshUserName"\
        &> >(print_error_stream) \
        1> /dev/null
    print $? "Add configs to enable SSH authentication" \
        || exit 1
